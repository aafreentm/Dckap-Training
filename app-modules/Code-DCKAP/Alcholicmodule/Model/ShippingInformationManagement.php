...
class ShippingInformationManagement
{
   ...
   public function __construct(
    ...
    $getdatass =  \Magento\Framework\App\ObjectManager::getInstance();
    $result = $this->jsonResultFactory->create();
    $store = $getdatass->get('Magento\Store\Model\StoreManagerInterface')->getStore();
     $cart = $getdatass->get('\Magento\Checkout\Model\Cart');
     ...
     $mystates = explode(",",$getstatesOptions);
           ...
           foreach($itemsCollection as $item){
            $_product_aattr = $getdatass->get('Magento\Catalog\Model\Product')->load($item->getProductId());
           ...
           foreach($mystates as $states_id){
                if($states_id)
                {
                   $region = $getdatass->create('Magento\Directory\Model\Region')->load($states_id);
